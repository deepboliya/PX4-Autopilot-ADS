...
//     PX4_INFO("CustomAttitudeControl::Run() called");
	...
	_last_run = now;
	// PX4_INFO("CustomAttitudeControl::Run() dt = %.6f seconds", (double)dt);

	// Current attitude quaternion
	const Quatf q_current(vehicle_attitude.q);

	// Get attitude setpoint - only update when new data available
	...
	if (_vehicle_attitude_setpoint_sub.updated()) {
		if (_vehicle_attitude_setpoint_sub.copy(&attitude_setpoint)) {
			...
			// Store thrust in persistent member variable
			_thrust_setpoint_body = Vector3f(attitude_setpoint.thrust_body);
		}
	} else {
		// No new setpoint - use last known attitude setpoint
		if (_vehicle_attitude_setpoint_sub.copy(&attitude_setpoint)) {
			q_setpoint = Quatf(attitude_setpoint.q_d);
		}
	}
	...
	// Use persistent thrust setpoint (essential for flight!)
	_thrust_setpoint_body.copyTo(rates_sp.thrust_body);
	// PX4_INFO("Using thrust body: [%.3f, %.3f, %.3f]", (double)rates_sp.thrust_body[0],
	// 	 (double)rates_sp.thrust_body[1], (double)rates_sp.thrust_body[2]);
	// View torque setpoints over ros2 topic instead of flooded console
